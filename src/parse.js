 ...
 * @return {Object}
 */
module.exports = function toEulerAngles(gyroscope, accelerometer, productId) {
  const now = Date.now();
  const dt = lastValues[productId].timestamp ? (now - lastValues[productId].timestamp) / 1000 : 0;
  ...
  lastValues[productId].alpha = (1 - zeroBias) * (lastValues[productId].alpha + gyroscope.z * dt);
  if (norm !== 0) {
    lastValues[productId].beta = bias * (lastValues[productId].beta + gyroscope.x * dt) + (1.0 - bias) * ((accelerometer.x * scale) / norm);
    lastValues[productId].gamma =
      bias * (lastValues[productId].gamma + gyroscope.y * dt) + (1.0 - bias) * ((accelerometer.y * -scale) / norm);
  }
  return {
    alpha:
      // ToDo: I could only get this to work with a magic multiplier (430).
      productId === 0x2006
        ? ((((-1 * (lastValues[productId].alpha * 180)) / Math.PI) * 430) % 90).toFixed(6)
        : ((((lastValues[productId].alpha * 180) / Math.PI) * 430) % 360).toFixed(6),
...
module.exports = function toEulerAnglesQuaternion(q) {
  const ww = q.w * q.w;
  const xx = q.x * q.x;
  const yy = q.y * q.y;
  const zz = q.z * q.z;
...
module.exports = function toQuaternion(gyro, accl, productId) {
  if (productId === 0x2006) {
...
module.exports = function parseDeviceInfo(rawData, data) {
...
module.exports = function parseInputReportID(rawData, data) {
  const inputReportID = {
...
module.exports = function parseTimer(rawData, data) {
  const timer = {
...
module.exports = function parseBatteryLevel(rawData, data) {
  const batteryLevel = {
...
module.exports = function parseConnectionInfo(rawData, data) {
...
module.exports = function parseButtonStatus(rawData, data) {
  const buttonStatus = {
...
module.exports = function parseCompleteButtonStatus(rawData, data) {
  const buttonStatus = {
...
module.exports = function parseAnalogStick(rawData, data) {
  const analogStick = {
...
module.exports = function parseAnalogStickLeft(rawData, data) {
...
module.exports = function parseAnalogStickRight(rawData, data) {
...
module.exports = function parseFilter(rawData, data) {
  const filter = {
...
module.exports = function parseVibrator(rawData, data) {
  const vibrator = {
...
module.exports = function parseAck(rawData, data) {
  const ack = {
...
module.exports = function parseSubcommandID(rawData, data) {
  const subcommandID = {
...
module.exports = function parseSubcommandReplyData(rawData, data) {
...
module.exports = function parseAccelerometers(rawData, data) {
...
module.exports = function parseGyroscopes(rawData, data) {
  const gyroscopes = [
    [
      {
...
module.exports = function calculateActualAccelerometer(accelerometers) {
...
module.exports = function calculateActualGyroscope(gyroscopes) {
